...
export class AutonomousMarketResearcher {
  ...
  async researchNiche(userId: string, niche: string): Promise<{
        ...
        if (platformListings && platformListings.length > 0) {
          for (const listing of platformListings.slice(0, 3)) {
            ...
            if (isHighConfidence) {
              ...
              try {
                const { dnaHuntOrchestrator } = await import('./dnaHuntOrchestrator.js');
                await dnaHuntOrchestrator.triggerHunt(userId, pTask.name, niche);
              } catch (huntErr: any) {
                console.error(`[MarketResearch] Failed to trigger hunt for ${pTask.name}:`, huntErr.message);
              }
            }
          }
        }
    ...
    // ── 4. Social Signals: TikTok & YouTube ──
    try {
      const socialSignals = await marketIntelligenceService.fetchTikTokTrends(niche, userId);
      if (socialSignals && socialSignals.length > 0) {
        for (const signal of socialSignals.slice(0, 3)) {
          const views = parseInt(String(signal.signals?.views || '0'));
          const confidence = (views > 50000) ? 0.9 : 0.75;
          await this.saveSignal({
            niche,
            platform: 'tiktok',
            signalType: 'social_heat',
            title: `TikTok Viral: ${signal.title}`,
            description: `Views: ${signal.signals?.views}, Likes: ${signal.signals?.likes}`,
            confidence,
            actionable: true,
          });
          output.signalsCreated++;
          if (confidence > 0.85) output.highConfidenceCount++;
        }
      }
    } catch (err: any) {
      console.error(`[MarketResearch] Social signals error for "${niche}":`, err.message);
    }

    // ── 5. Escalation: Trigger Approval Gate for high-confidence signals ──
      ...
      try {
        const approval = await approvalService.createRequest(
          ...
          `AI detected ${output.signalsCreated} signal(s) in "${niche}", ${output.highConfidenceCount} high-confidence. Review recommended.`,
          {
            ...
            highConfidenceCount: output.highConfidenceCount,
