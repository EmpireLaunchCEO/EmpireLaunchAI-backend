...
const { strategySuggestions, userSettings, nicheDnaRepository, marketSignals } = schema;
...
export class MassDnaHarvestWorker {
  ...
  private async harvestNiche(niche: string, model: any): Promise<void> {
    // Load triggers for this niche if available
    const targets = this.loadPhase5Targets();
    const nicheConfig = targets?.platforms;
    ...
    if (listings && listings.length > 0) {
      ...
      for (const listing of topListings) {
        try {
          // Check triggers
          if (nicheConfig?.etsy) {
            await this.checkViralSignals('etsy', listing, nicheConfig.etsy.ingestion_filters?.critical_alert_triggers);
          }
    ...
    try {
      ...
      if (tiktokTrends && tiktokTrends.length > 0) {
        ...
        for (const trend of topTikTok) {
          try {
            // Check triggers
            if (nicheConfig?.tiktok) {
              await this.checkViralSignals('tiktok', trend, nicheConfig.tiktok.ingestion_filters?.critical_alert_triggers);
            }
  ...
  private loadPhase5Targets(): any {
    try {
      const targetsPath = '/home/team/shared/Phase5_Harvest_Targets.json';
      if (!fs.existsSync(targetsPath)) return null;
  ...
  private async checkViralSignals(platform: string, listing: MarketListing, triggers: any): Promise<void> {
    if (!triggers) return;
    ...
    if (platform === 'etsy') {
      const inBasketCount = parseInt(listing.signals?.inBasket?.replace(/[^0-9]/g, '') || '0');
      if (triggers.bestseller && listing.isBestSeller) {
        isViral = true;
        reason = `Best seller detected on Etsy: ${listing.title}`;
      } else if (triggers.basket_count_threshold && inBasketCount >= triggers.basket_count_threshold) {
        isViral = true;
        reason = `High demand: ${inBasketCount} people have this in their basket on Etsy.`;
      }
    } else if (platform === 'tiktok') {
      const views = this.parseCount(listing.signals?.views);
      const likes = this.parseCount(listing.signals?.likes);
      
      if (triggers.viral_outlier_views_threshold && views >= triggers.viral_outlier_views_threshold) {
        isViral = true;
        reason = `Viral TikTok alert: ${listing.signals?.views} views for "${listing.title}"`;
      } else if (triggers.minimum_views && views >= triggers.minimum_views) {
        const ratio = views > 0 ? likes / views : 0;
        if (triggers.like_to_view_ratio_min && ratio >= triggers.like_to_view_ratio_min) {
          isViral = true;
          reason = `High engagement TikTok: ${Math.round(ratio * 100)}% like-to-view ratio.`;
        }
      }
    }

    if (isViral) {
      console.log(`[MassDNAHarvest] Viral Signal detected! platform=${platform}, reason=${reason}`);
      await this.createStrategySuggestionFromViralSignal(listing, platform, reason);
    }
  }
  ...
  private parseCount(val: any): number {
    ...
    const clean = val.toUpperCase().replace(/[^0-9.KMB]/g, '');
  ...
  private async createStrategySuggestionFromViralSignal(listing: MarketListing, platform: string, reason: string): Promise<void> {
    // Find users who might be interested in this niche
    // For now, we'll target users whose businessNiche matches or is similar
    const niche = listing.tags?.[0] || 'digital products';
    
    const relevantUsers = await db.select()
      .from(userSettings)
      .where(sql`${userSettings.businessNiche} LIKE ${'%' + niche + '%'}`);

    for (const user of relevantUsers) {
      await db.insert(strategySuggestions).values({
        id: uuidv4(),
        userId: user.userId,
        type: 'TREND_PIVOT',
        title: `Viral Trend Alert: ${listing.platform}`,
        suggestion: `We detected a viral signal for **${listing.title}** on ${listing.platform}. You should consider creating a similar product or content format to ride this wave.`,
        reasoning: reason,
        platformInsights: {
  ...
  /**
   * Load new research targets from Market Researcher's JSON files.
   * Merges any new niches/platforms into the harvest queue.
   */
  private async loadResearchTargets(): Promise<void> {
    try {
      // 1. Load from RESEARCH_TARGETS.json
      const researchTargetsPath = '/home/team/shared/RESEARCH_TARGETS.json';
      if (fs.existsSync(researchTargetsPath)) {
        const raw = fs.readFileSync(researchTargetsPath, 'utf-8');
        const targets = JSON.parse(raw);
        this.ingestNiches(targets.newNiches);
        this.ingestPlatforms(targets.newPlatforms);
        
        // Clear the new items so we don't re-add them
        targets.newNiches = [];
        targets.newPlatforms = [];
        targets.lastUpdated = new Date().toISOString();
        fs.writeFileSync(researchTargetsPath, JSON.stringify(targets, null, 2));
      }

      // 2. Load from Phase5_Harvest_Targets.json
      const phase5TargetsPath = '/home/team/shared/Phase5_Harvest_Targets.json';
      if (fs.existsSync(phase5TargetsPath)) {
        const raw = fs.readFileSync(phase5TargetsPath, 'utf-8');
        const targets = JSON.parse(raw);
        
        // Phase 5 targets use a different structure (platforms mapping)
        const phase5Niches: string[] = [];
        if (targets.platforms) {
          for (const key in targets.platforms) {
            const p = targets.platforms[key];
            if (p.search_queries) phase5Niches.push(...p.search_queries);
            if (p.search_keywords) phase5Niches.push(...p.search_keywords);
          }
        }
        this.ingestNiches(phase5Niches);
      }
  ...
  private ingestNiches(niches: any[]): void {
    if (!niches || !Array.isArray(niches)) return;
    for (const newNiche of niches) {
      ...
      if (nicheStr && !HARVEST_NICHES.includes(nicheStr)) {
        ...
        console.log(`[MassDNAHarvest] Added new research target niche: "${nicheStr}"`);
      }
    }
  }

  private ingestPlatforms(platforms: any[]): void {
