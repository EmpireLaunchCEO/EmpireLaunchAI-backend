...
export class PlatformRouter {
  ...
  private async handleSystemeIo(action: string, params: any): Promise<ExecutionResult> {
    try {
      ...
      if (!creds) return { success: false, error: 'Systeme.io not connected' };
      
      switch (action) {
        case 'create_contact':
          const contact = await systemeIoService.createContact(creds.api_key, params.contact);
          return { success: true, data: { contact } };
        case 'add_tag':
          const tag = await systemeIoService.addTag(creds.api_key, params.tagName);
          return { success: true, data: { tag } };
        case 'get_newsletters':
          const newsletters = await systemeIoService.getNewsletters(creds.api_key);
          return { success: true, data: { newsletters } };
        default:
          return { success: false, error: `Unknown Systeme.io action: ${action}` };
      }
    } catch (error: any) {
  ...
  private async handleGoDaddy(action: string, params: any): Promise<ExecutionResult> {
    try {
      ...
      if (!creds) return { success: false, error: 'GoDaddy not connected' };

      switch (action) {
        case 'search_domain':
          const availability = await goDaddyService.searchDomain(creds.api_key, creds.api_secret, params.domain);
          return { success: true, data: { availability } };
        case 'update_dns':
          const result = await goDaddyService.updateDns(creds.api_key, creds.api_secret, params.domain, params.records);
          return { success: true, data: { result } };
        default:
          return { success: false, error: `Unknown GoDaddy action: ${action}` };
      }
    } catch (error: any) {
